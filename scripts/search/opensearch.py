#!/usr/bin/env python3
"""opensearch.py — index the 9/11 records into local OpenSearch and query them (hybrid).

Python stdlib only (urllib), against http://127.0.0.1:9200 (docker-compose.opensearch.yml).
Local only; never contacts the portal.
...
One OpenSearch document per PAGE (id "<bates_start>_p<page>"):
  ...
  pdf_size, ocr_status (ok|empty|junk)
...
Commands:
  setup     create the index (idempotent; --recreate drops it first) and the hybrid search pipeline
...
def call(method: str, path: str, body=None, ndjson: str | None = None, ok404=False):
    ...
    req = urllib.request.Request(OS + path, data=data, method=method, headers={"Content-Type": ctype})
    try:
...
def setup(recreate: bool) -> None:
    ...
    print(json.dumps({"index": INDEX, "pipeline": PIPELINE, "cluster": call("GET", "/")["version"]["number"]}))
    ...
    for f in sorted(TEXT.rglob("*.pages.jsonl")):
        ...
        for line in f.open():
            ...
            src = {
                ...
                "pdf_size": int(m.get("pdf_size") or 0) or None, "ocr_status": status.get((doc, page), "empty"),
