...
export async function generateMetadata({params}:Props):Promise<Metadata> {
  ...
  return {title:file?`${file.place.label} · Building file`:'Building not found',description:'Source pages, test candidates and documents across all boxes for this building. Machine-extracted matches link to the record.',alternates:{canonical:file?buildingUrl(file.place):`/building/${encodeURIComponent(bin)}`},...(!file?{robots:{index:false,follow:false}}:{})};
}
export default async function BuildingPage({params}:Props) {
  ...
  return <><Header active="/map"/><main id="main" className={styles.building}>
