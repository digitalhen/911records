...
interface Related extends Source { cross:number;score:number; box:string|null;shared_entities:number;shared_topics:number;same_box:boolean }
export async function RelatedRecords({doc}:{doc:string}) {
 let rows:Related[];
 try {rows=await queryReadSafe<Related>(`SELECT r.other AS doc,p.page,r.score,r.cross,d.box,
