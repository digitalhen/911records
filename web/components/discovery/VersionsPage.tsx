...
export default async function VersionsPage({params,searchParams}:{params:Params;searchParams:Promise<SearchParamsInput>}) {
 ...
 return <Shell title="Copies and versions" eyebrow="Near-duplicate records" active="/search"><Link href={`/doc/${encodeURIComponent(doc)}`}>← Back to record</Link><p>Copies are not independent tests. A high similarity score can also match a reused form; it does not establish which copy came first or whether their findings agree.</p>{!copies.length?<p>No near-duplicates are indexed for this record.</p>:<><ul>{copies.map(c=><li key={c.doc}><Link href={c.status==='removed'?`/doc/${encodeURIComponent(c.doc)}`:`/doc/${encodeURIComponent(doc)}/versions?copy=${encodeURIComponent(c.doc)}`}>{c.doc}{c.status==='removed'?' · removed by the City':` · Box ${c.box||'unavailable'}`}</Link>{c.status!=='removed'&&<><span> · similarity {Number(c.score).toFixed(3)} (not a probability)</span><Extraction source={{doc:c.doc,page:1,confidence:null}}/></>}</li>)}</ul>{copy&&<form className={styles.search} method="get"><input type="hidden" name="copy" value={copy.doc}/><label htmlFor="comparison-page">Page in each copy</label><input id="comparison-page" name="page" type="number" min="1" max={Math.max(primary.page_count||1,copy.page_count||1)} defaultValue={page}/><button className="button">Compare pages</button></form>}</>}
