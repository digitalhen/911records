...
import { SUGGESTED_QUESTION_GROUPS as groups } from '@/lib/suggestedQuestions';
import { WhatOthersAreReading } from '@/components/reading/WhatOthersAreReading';
import styles from './home.module.css';
// B15: the question list itself moved to lib/suggestedQuestions.ts (no
// JSX/CSS import there, so web/scripts/check-suggestions.ts can import it
// under plain `tsx`); re-exported here so nothing else needs to change.
export { SUGGESTED_QUESTIONS } from '@/lib/suggestedQuestions';
function EntryPoints() { return <><section><h2>Start with a question</h2><p className="small muted">No account needed to read</p>{groups.map(([title, ...questions]) => <div key={title}><h3>{title}</h3>{questions.map(q => <a className="question-link" href={`/ask?q=${encodeURIComponent(q)}&mode=question`} key={q}>{q}<span aria-hidden><AiMark /> ↗</span></a>)}</div>)}</section><section><h2>Find what you did not know to ask</h2><Link className="question-link" href="/first-responders">For first responders <span>→</span></Link><Link className="question-link" href="/browse">Browse in collection order <span>→</span></Link><Link className="question-link" href="/topics">Follow a subject · Topic map <span>→</span></Link><Link className="question-link" href="/entities">Labs, offices, contractors and substances <span>→</span></Link></section><section><h2>Reading these records</h2><p>Search uses imperfect OCR. Check the page image when a date or reading matters. Machine-written answers can misread scans and do not determine eligibility for a claim.</p><p><Link href="/personal-information">Personal-information policy</Link></p></section></>; }
