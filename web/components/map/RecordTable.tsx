...
export default function RecordTable({ rows, compact = false }: { rows: Candidate[]; compact?: boolean }) {
  ...
  return <div className={styles.records}>
    ...
    <p className={styles.note}>Candidates extracted by machine can misread scans. Dates, substances, measurements and labs below occur on the same page; their relationship is not established. Verify the page.</p>
    {!rows.length ? <p>No test candidates in this selection.</p> : <div className={styles.tableWrap}><table className={compact ? styles.compactTable : styles.table}>
      ...
      <tbody>{sorted.map(r=><tr key={`${r.doc}:${r.page}`}>
        ...
        <td><strong>{r.contaminants.join(', ') || 'Substance not extracted'}</strong>
          <p>{r.measurements.length ? r.measurements.join(' · ') : 'Numeric value not extracted'}{!r.measurements.length && r.units.length ? ` · units: ${r.units.join(', ')}` : ''}</p>
          ...
          <small>Machine-extracted · confidence {r.confidence === null ? 'not available' : r.confidence.toFixed(2)} · <a href={pageUrl(r)}>verify</a></small>
        </td>
      </tr>)}</tbody>
    </table></div>}
  </div>;
