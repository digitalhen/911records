...
export async function informationSitemapPaths(): Promise<string[]> {
  ...
  return ['/first-responders', '/downloads', '/contradictions', '/browse', '/changes', '/personal-information', '/privacy', '/terms', '/about', '/reading', ...rows.map(row => `/changes/${encodeURIComponent(formatDate(row.date))}`)];
}
