...
export const outputSchemas = {
  search_records: {
    hits: z.array(z.object({ doc: id, page, bates: id, agency: text, machine_extracted_title: text, url: link })),
  ...
  get_document: {
    ...
    url: link, official_url: link.nullable(), pdf_url: link.nullable(),
    pages: z.array(z.object({ page, url: link })), next_page: page.nullable(),
  },
  get_page: {
    ...
    next_offset: nextOffset, total_chars: count, url: link,
    scan_url: link.nullable(), official_url: link.nullable(), note: z.string(),
  },
  browse_collection: {
    documents: z.array(z.object({ doc: id, agency: text, volume: text, box: text, page_count: count.nullable(), url: link })),
    next_after: id.nullable().describe('Pass as after to retrieve the next batch; null when complete.'),
  },
  get_changes: {
    changes: z.array(z.object({ date: z.iso.date().describe('Catalog capture date, not document date.'), doc: id, kind: z.string() })),
